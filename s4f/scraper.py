...
class SportsScraper:
    def __init__(self):
        ...
        # Saves to s4f folder relative to repo root
        self.output_dir = "s4f"
    ...
    def run(self):
        try:
            with httpx.Client(headers=self.headers, timeout=30.0) as client:
                ...
                group_map = {str(g['id']).strip(): str(g['name']).strip() for g in groups_res if isinstance(g, dict)}
        ...
        # Added timestamp to force Git to see a change every hour
